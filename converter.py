...
class TranslateConverter(PDFConverterEx):
    def __init__(
        ...
        if not envs:
            envs = {}
        for translator in [IndicTransTranslator, GoogleTranslator, BingTranslator, DeepLTranslator, DeepLXTranslator, OllamaTranslator, XinferenceTranslator, AzureOpenAITranslator,
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        total_chunks = len(sstk)
        non_empty_chunks = sum(1 for s in sstk if s.strip() and not re.match(r"^\{v\d+\}$", s))
        if non_empty_chunks > 0:
            log.info(f"Translating {non_empty_chunks} text chunks (out of {total_chunks} total)...")
        ...
        if non_empty_chunks > 0:
            log.info(f"✓ Completed translation of {non_empty_chunks} chunks")
            log.info("Now rendering PDF with translated content...")
            
            # Light cache cleanup - don't delete the model as it might be needed during rendering
            try:
                import torch
                import gc
                ...
                gc.collect()
                log.debug("Cleared GPU/CPU cache after translation")
            except Exception as e:
                log.debug(f"Cache cleanup skipped: {e}")

        ############################################################
        # C. 新文档排版
        def raw_string(fcur: str, cstk: str):  # 编码字符串
            if fcur == self.noto_name:
                # For Indic fonts, try has_glyph() first, but fallback to Unicode if missing
                # This ensures proper glyph mapping when available, but preserves diacritics when glyphs are missing
                result = []
                for c in cstk:
                    char_code = ord(c)
                    glyph_code = self.noto.has_glyph(char_code)
                    if glyph_code == 0:
                        # If glyph missing, use Unicode code point directly (important for Indic diacritics)
                        # This ensures guninthalu, othulu, deergaalu are preserved
                        glyph_code = char_code
                        if log.isEnabledFor(logging.DEBUG):
                            log.debug(f"Glyph missing for U+{char_code:04X}, using Unicode directly")
                    result.append("%04x" % glyph_code)
                return "".join(result)
        ...
        LANG_LINEHEIGHT_MAP = {
            ...
            # Indic languages - increased line height for better diacritic spacing
            "te": 1.3, "kn": 1.3, "hi": 1.3, "ta": 1.3, "mr": 1.3, "gu": 1.3,
            "bn": 1.3, "or": 1.3, "pa": 1.3, "ml": 1.3, "as": 1.3
        }
    ...
    def close(self):
        """Clean up resources, especially model instances for IndicTrans"""
        try:
            import torch
            import gc
            # Clean up translator model if it exists
            if hasattr(self, 'translator') and self.translator is not None:
                if hasattr(self.translator, 'model_instance') and self.translator.model_instance is not None:
                    # Move to CPU and release
                    try:
                        if torch.cuda.is_available():
                            self.translator.model_instance = self.translator.model_instance.cpu()
                        del self.translator.model_instance
                        self.translator.model_instance = None
                    except Exception as e:
                        log.debug(f"Error cleaning up model_instance: {e}")
            ...
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            gc.collect()
            log.debug("Cleaned up translator resources")
        except Exception as e:
            log.debug(f"Error during close(): {e}")
